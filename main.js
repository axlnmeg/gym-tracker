...
function renderLogin() {
  ...
  // Social auth — auto-register if not exists, then login
  $('#google-btn').addEventListener('click', () => {
    const email = 'user@gmail.com';
    if (!Store.emailExists(email)) {
      Store.registerUser({ name: 'Athlete', email, password: 'google_oauth' });
      Store.clearAllData();
      Store.initNewUserData();
    }
    Store.setUser({ email, name: 'Athlete' });
    Router.navigate('dashboard');
  });
  $('#apple-btn').addEventListener('click', () => {
    const email = 'user@icloud.com';
    if (!Store.emailExists(email)) {
      Store.registerUser({ name: 'Athlete', email, password: 'apple_oauth' });
      Store.clearAllData();
      Store.initNewUserData();
    }
    Store.setUser({ email, name: 'Athlete' });
